...
//                VolleyUtil.getInstance(TestVisitActivity.this).visit(Request.Method.POST, url, "test", argMap);
                VolleyTask volleyTask = new VolleyTask(BaseApplication.self(), "test", activityTitleSampleA, true, url, argMap, Request.Method.POST, null);
                volleyTask.setOnVisitResultListener(new IOnVisitResultListener<ResultEntity>() {
                    @Override
                    public void onSuccess(ResultEntity res) {
                        LogUtil.e("resultEntity= "+res.getCode());
                    }
