...
public class KeepWatchService extends Service {
    ...
    protected final int onStart(Intent intent, int flags, int startId) {
        ...
        try {
            LogUtil.e("--------------starting KeepWatchService--------------"+ DaemonHelper.sInitialized);
            ...
            if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.N) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2)
                    startService(new Intent(DaemonHelper.sApp, WatchDogNotificationService.class));
            }
            ...
            sSubscription = Observable.interval(DaemonHelper.getWakeUpInterval(), TimeUnit.MILLISECONDS)
                    .subscribe(new Action1<Long>() {
                        public void call(Long aLong) {
                            KeepWatchService.this.startService(new Intent(DaemonHelper.sApp, DaemonHelper.sServiceClass));
                        }
            ...
            getPackageManager().setComponentEnabledSetting(new ComponentName(getPackageName(), DaemonHelper.sServiceClass.getName()),
                    PackageManager.COMPONENT_ENABLED_STATE_ENABLED, PackageManager.DONT_KILL_APP);
            LogUtil.e("--------------start KeepWatchService success--------------");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return START_STICKY;
    ...
    protected void onEnd(Intent rootIntent) {
        try {
            ...
            startService(new Intent(DaemonHelper.sApp, KeepWatchService.class));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     *
     * 因 KeepWatchService 运行在 :watch 子进程, 请勿在主进程中直接调用此方法.
    ...
    public static void cancelJobAlarmSub() {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                JobScheduler scheduler = (JobScheduler) DaemonHelper.sApp.getSystemService(JOB_SCHEDULER_SERVICE);
                scheduler.cancel(HASH_CODE);
            } else {
                AlarmManager am = (AlarmManager) DaemonHelper.sApp.getSystemService(ALARM_SERVICE);
    ...
    public static class WatchDogNotificationService extends Service {
        ...
        public int onStartCommand(Intent intent, int flags, int startId) {
            try {
                startForeground(KeepWatchService.HASH_CODE, new Notification());
                stopSelf();
            } catch (Exception e) {
                e.printStackTrace();
            }
