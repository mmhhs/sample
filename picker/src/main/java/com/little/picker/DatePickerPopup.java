 ...
 */
public class DatePickerPopup extends PopupWindow implements OnClickListener {
    ...
    private boolean showHour = false;//是否显示小时区
    private boolean autoDismiss = false;//是否确认后自动关闭弹窗
    ...
    public static class Builder{
        ...
        private int colorConfirm = Color.parseColor("#396bef");
        private int btnTextSize = 16;//text btnTextsize of cancel and confirm button
        private int viewTextSize = 25;
        private boolean showHour = false;//是否显示小时区（第四块）
        private boolean autoDismiss = false;//是否确认后自动关闭弹窗
        ...
        public Builder showHour(boolean showHour){
    ...
    public DatePickerPopup(Builder builder){
        ...
        this.showHour = builder.showHour;
...
//                    initHourPickerView();
    ...
    private void initHourPickerView() {
        ...
        for (int i = 8; i <= 17; i++) {
            hourList.add(format2LenStr(i)+mContext.getString(R.string.picker_hour));
        }
    ...
    public void onClick(View v) {
        ...
        if (v == contentView || v == cancelBtn) {
            ...
            if (null != mListener) {
                ...
                if (showHour){
                    mListener.onDatePickCompleted(year, month, day,hour.replace(mContext.getString(R.string.picker_hour),""), sb.toString());
                }
            }
            if (autoDismiss){
                dismissPopWin();
    ...
    public interface OnDatePickedListener {
        ...
        void onDatePickCompleted(int year, int month, int day,
                                 String dateDesc);
        void onDatePickCompleted(int year, int month, int day, String hour,
                                 String dateDesc);
    }
}
